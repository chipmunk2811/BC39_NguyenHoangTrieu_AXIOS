...
function addUser() {
    userService.getListUserAPI()
        .then(function (result) {
            ...
            isValid &= vaLiDaTion.checkSpace(taiKhoan, "spTaiKhoan", "(*) Không Để Tài Khoản Ảnh Trống")
                && vaLiDaTion.trungTK(taiKhoan, "spTaiKhoan", "Tài Khoản Của Bạn Bị Trùng", result.data);

            if (!isValid) return;
            // ADD User
            var user = new FormUser("", taiKhoan, hoTen, matKhau, email, hinhAnh, loaiND, ngonNgu, moTa);
